...
public class SendMail extends HtmlReportSupport {
	...
	public static void attachReportsToEmail() throws Exception {
		...
		String strZipFilePath = "Results\\"
				+ strReportsFolder
				+ TestEngine.filePath().split(strReportsFolder)[2].replace(
						"\\", "") + ".Zip";
		Zip.zipFolder(TestEngine.filePath(), strZipFilePath);

		String[] to = { config.getProperty("ToAddresses") };
		String[] cc = { config.getProperty("ToAddresses") };
		String[] bcc = {};
		...
		SendMail.sendMail(
				...
				strZipFilePath,
				strReportsFolder
						+ TestEngine.filePath().split(strReportsFolder)[2]
								.replace("\\", "") + ".Zip");
...
//		String strReportsFolder = "Firefox";
...

		{
			...
			msg.setFrom(new InternetAddress(config.getProperty("ToAddresses")));
