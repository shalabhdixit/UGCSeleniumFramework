...
public class ResubmitQuote_UnSuccessfulQuote extends BusinessFunctions {
	@Test(dataProvider = "get_TestData")
	public void Resubmit_Quote_UnSuccessful_Quote(String borrowerLastName,
		...
		try {
			...
			// Create New Quote [ Negative Test ]
			...
			if (fillNewQuoteForm(borrowerLastName, lenderLoanNumber,
					...
					financedPremium, upFrontPremium, postPay, "BPSR")) {
				SuccessReport("Create New Quote", "New Quote form filled successfully");
			} else {
				failureReport("Create New Quote", "Unable to fill New Quote form");
			}
			...
			if (isElementPresent(MIGNG_NewQuote.MIGNG_msgUnsuccessfulQuoteHead,	"Unsuccessful Quote")
					&& driver.findElement(MIGNG_NewQuote.MIGNG_msgUnsuccessfulQuoteHead)
							.getText().equalsIgnoreCase(MIGNG_Validation.msgUnsuccessfulQuoteHead)) {

				// validate message
				if (isElementPresent(MIGNG_NewQuote.MIGNG_msgUnsuccessfulQuote,	"Unsuccessful Quote")
						&& driver.findElement(MIGNG_NewQuote.MIGNG_msgUnsuccessfulQuote).getText()
								.equalsIgnoreCase(MIGNG_Validation.msgUnsuccessfulQuoteCreditScore)) {
					SuccessReport("Unsuccessful Quote validation",	"Unsuccessful Quote");
				} else {
					failureReport("Unsuccessful Quote validation", "Unsuccessful Quote validation failed. Msg - "
									+ driver.findElement(MIGNG_NewQuote.MIGNG_msgUnsuccessfulQuote).getText());
				}
				SuccessReport("Unsuccessful Quote validation", "Unsuccessful Quote");
			} else {
				failureReport("Unsuccessful Quote validation", "Unsuccessful Quote validation failed");
			}
...
//			browser = configProps.getProperty("browserType");
	...
	public Object[][] get_TestData() throws Exception {
		return Data_Provider.getTableArray("ResubmitQuote_Unsuccessful");
	}
}
