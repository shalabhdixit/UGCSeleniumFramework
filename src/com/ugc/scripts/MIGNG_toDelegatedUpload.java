...
public class MIGNG_toDelegatedUpload extends BusinessFunctions {

	@Test(dataProvider = "get_TestData")
	public void MIGNG_To_Delegated_Upload (String First, String Last, String SSN, String DOB, String NoYears,
		...
		try {
			...
			char[] charsi = "1234567890123456789012345678901234567890".toCharArray();
			...
			SSN = Ssn.toString();
			SSN1 = Ssn1.toString();	
			if (MIGNG_toDelegatedUpload(robot,stringSelection)) {
			...
			if (MIGNG_Rapid_NewApplicationupload(First, Last, SSN, DOB, NoYears, Mths, StreetAddress, CITY, state, zip,
					...
					BalloonTerm, Subordinate, EstimatedCashtoClose, LiquidAssets, Hazardins, Coverage)) {
			...
			if (MIGNG_toDelegatedUpload1(robot,stringSelection)) {
				SuccessReport("Create New Application", "New Application Uploaded successfully");
			} else {
				failureReport("Logout MIGuide NG", "Unable to Upload File");
			}
			driver.switchTo().frame("MainFrame");
			assertText(MIGNG_RAPidLink.MIGNG_RAPidLink_RapidVerify, "RAPidLink");
			assertText(MIGNG_RAPidLink.MIGNG_RAPidLink_errorVerify, "Error Verified");
			if (MIGNG_Rapid_NewApplicationupload1(First1, Last1, SSN1, DOB, NoYears, Mths, StreetAddress1, CITY, state, zip,
					Monthly_Income, Gender, Ethnicity, OtherMonthlyPayments, Race, CreditScore, CreditRepository,
